...
lemma isItemLift_of_isInsert_of_lt {t : PTerm S} {n : ℕ} {Γ Δ Δ' : PCtx S} :
    isInsert Γ t n Δ Δ' → ∀ {k}, (k < n → ∀ {u}, (u ↓ k sub Δ) →
      (u ↑ 1 # n) ↓ k sub Δ') := by
  intro h k hk u hu
...
lemma cons_inv (h : Derivable (A :: Γ ⊢ ⬝)) :
...
example (h : Derivable ([] ⊢ A : !s)) : A = A ↑ 1 := by
  ...
  . obtain ⟨_, ⟨_, ⟨_, ⟨_, hl⟩⟩⟩⟩ := exists_of_var_typing h
    cases hl
  . simp
